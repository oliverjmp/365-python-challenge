...
def test_save_cookies_exception(monkeypatch):
    """Fuerza una excepción en el guardado para cubrir el bloque except (líneas 39-40)."""
    manager = SessionPersistence(cookie_file="test_cookies.pkl", headless=True)
    
    class MockBrowserError:
        def get(self, url):
            raise Exception("Error de conexión al guardar")
        def quit(self):
            pass

    monkeypatch.setattr("src.session_persistence.ChromeDriverManager.install", lambda *args, **kwargs: "path")
    monkeypatch.setattr("src.session_persistence.webdriver.Chrome", lambda *args, **kwargs: MockBrowserError())

    success = manager.save_cookies("http://invalid.url.test.local")
...
def test_load_session_exception(monkeypatch):
    """Fuerza una excepción en la carga para cubrir el bloque except (líneas 64-65)."""
    manager = SessionPersistence(cookie_file="test_cookies.pkl", headless=True)
    
    # Creamos el archivo para que pase la verificación de existencia, pero fallará al cargar/añadir cookies
    with open("test_cookies.pkl", "wb") as f:
        pickle.dump("datos_corruptos", f)

    class MockBrowserLoadError:
        def get(self, url):
            pass
        def add_cookie(self, cookie):
            raise Exception("Error al inyectar cookie")
        def quit(self):
            pass

    monkeypatch.setattr("src.session_persistence.ChromeDriverManager.install", lambda *args, **kwargs: "path")
    monkeypatch.setattr("src.session_persistence.webdriver.Chrome", lambda *args, **kwargs: MockBrowserLoadError())
